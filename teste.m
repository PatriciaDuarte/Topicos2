...
[vlrands50] = 512*rand(1,50);

[vlrands50] = granulacao(vlrands50);
...
for i=1:max(size(vlrands50))
  ...
  x = vlrands50(i);
  y = -abs(x.*(sin(sqrt(abs(x)))));
  plot(x, y,'-ro','LineWidth',0.5,'MarkerEdgeColor','r','MarkerFaceColor','r','MarkerSize',5);
end;

decimais = 0.0:0.5:511.5;
decimais = decimais';

#indivbin=dec2bin(342,10);
...
%gerando pares para o cruzamento
  ...
  for i=1:25
   ...
   %               
   fxvlrands50=sort(fxvlrands50);
   [elitismo] = [fxvlrands50(1:15)];
    
   vlrands50 = nvovlrands50;
   fxvlrands50 = sort(fxvlrands50);
    
    b=1;
   for a=1:15
      if(fxvlrands50(a)<elitismo(b))
        [fxvlrands50(a)] = [elitismo(b)];
      break;
    end;
    
  %                     
  end;
  [vlrands50] = mutacao(vlrands50, decimais);
end;
...
for i=1:max(size(vlrands50))
  ...
  x = vlrands50(i);
  y = -abs(x.*(sin(sqrt(abs(x)))));
  plot(x, y,'-ro','LineWidth',0.5,'MarkerEdgeColor','k','MarkerFaceColor','k','MarkerSize',5);
end;
end;